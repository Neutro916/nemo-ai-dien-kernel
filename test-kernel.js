...
const tasks = [
  { id: 'd1', type: 'docker_mayhem', data: { command: 'docker ps -a' } },
  { id: 'd2', type: 'docker_mayhem', data: { command: 'docker logs nemo-core' } },
  { id: 'd3', type: 'docker_mayhem', data: { command: 'docker exec -it nemo-core bash' } }
];

tasks.forEach((task, i) => {
  console.log(`\n[${i+1}] ${task.data.command}`);
...
let initial = agent.status().kernel.coherence;
...
let final = agent.status().kernel.coherence;
...
const { FrequencyKernel, RodinMath } = require('./src/agents/ai_dien/kernel');
